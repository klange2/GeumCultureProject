...
#Saddle is more phylogenetically distant than soddie even with less richness
...
ses.mpd.result.sum <- ses.mpd(dat.comm.sum[,1:35], phydist, null.model="taxa.labels", abundance.weighted=F, runs=999)
ses.mpd.result.sum
...
labels$Genus[is.na(labels$Genus)] <- "Unknown"
labels$Species[is.na(labels$Species)] <- "sp."
labels <- labels%>%
  unite(GenusSpecies, Genus, Species)
labels$OTUGS <- paste(labels$OTU, labels$GenusSpecies, sep="_")

matrix <- dat.comm.sumt[1:35,]
...
trimmednwk$tip.label <- labels[[4]][match(trimmednwk$tip.label, labels[[1]])]
...
tree <- plotTree.datamatrix(trimmednwk,tree.data,fsize=0.5,palettes=c("YlOrRd","PuBuGn"),yexp=1.12,xexp=2)
