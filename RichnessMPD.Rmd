...
levels(richness$Site) <- c("Soddie", "Saddle")
...
rm5 <- ggplot(richness,aes(x=Community.x,y=SR,color=Community.x,group=Community.x))+
  ...
  theme_classic()+
  theme(line=element_line(linewidth = .3),text=element_text(size=12),strip.background = element_rect(colour="white", fill="white"),axis.line=element_line(color="gray30",linewidth = .5),legend.position = "none",strip.placement = "outside",axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),panel.background = element_rect(fill = NA, color = "gray50"))+
  facet_wrap(vars(Site),strip.position = "bottom")+
  theme(strip.text.x = element_blank())
#dev.off()

ggsave("richnessplot.png",rm5)
...
m1<-nlme::gls(SR~Community.x*Site,data=dat6,na.action=na.omit)
...
levels(even$Site) <- c("Soddie", "Saddle")
...
``` {r}
m1<-nlme::gls(data_evenness~Community*Site,data=data_alphadiv,na.action=na.omit)
anova(m1,type="marginal")
#statistically different
...
levels(shan$Site) <- c("Low Elevation", "High Elevation")
#levels(shan$Site)[levels(shan$Site =="Soddie")]<-"Low Elevation"
#levels(shan$Site)[levels(shan$Site =="Saddle")]<-"High Elevation"
...
ggsave("shannonplot.png",shann)

```

##Stats
``` {r}
m1<-nlme::gls(data_shannon~Community*Site,data=data_alphadiv,na.action=na.omit)
anova(m1,type="marginal")
#statistically different
...
#divplot <- ggarrange(rm5, ev, shan, ncol = 1)
divplot <- (rm5 / ev / shann)

ggsave("divplot.png",divplot,width = 8, height = 12)
...
# 
# #just by community
# mpdcom <- ggplot(mpdmean,aes(x=Community.x,y=mean,color=Community.x))+
#   labs(x = "",y="MPD") +
#   theme_classic()+
#   theme(line=element_line(linewidth=.3),text=element_text(size=12),strip.background = element_rect(colour="white", fill="white"),axis.line=element_line(color="gray30",linewidth=.5),legend.position = "none",strip.placement = "outside",panel.spacing=unit(0,"cm"),axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),panel.background = element_rect(fill = NA, color = "gray50"))+
...
levels(mpd$Site) <- c("Low Elevation", "High Elevation")
...
levels(mntd$Site) <- c("Low Elevation", "High Elevation")
...
ggsave("MPD,MNTDplot.png",MDplot)
...
write.csv(mat,"Treemat.csv")
tree.data <- read.csv("Treemat.csv", row.names=2, stringsAsFactors = TRUE)
tree.data$X <- NULL
...
tree <- plotTree.datamatrix(cleanedtree_v2,tree.data,fsize=0.5,palettes=c("YlOrRd","PuBuGn"),yexp=1.05,xexp=1.65)
leg <- legend(x="topright",names(tree$colors$DM),cex=0.7,pch=22,pt.bg=tree$colors$DM,pt.cex=1.5,bty="n",title="Low Elevation")
#leg2 <- legend(x="bottomright",names(tree$colors$DM_Q),cex=0.7,pch=22,pt.bg=tree$colors$DM_Q,pt.cex=1.5,bty="n",title="Saddle")
leg2 <- legend(x=(leg$rect$left), y=(leg$rect$top-leg$rect$h),names(tree$colors$` `),cex=0.7,pch=22,pt.bg=tree$colors$` `,pt.cex=1.5,bty="n",title="High Elevation")
