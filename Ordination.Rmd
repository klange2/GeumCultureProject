...
ord2 <- plot_grid(plot1, plot2, nrow = 1, labels=c("A. Composition","B. MPD"),label_fontface = "plain",hjust=c(-.5,-1.),vjust=1,rel_widths = c(1,1.4))
#dev.off()
...
data_richness <- estimateR(div[,3:35]) 
...
data_evenness <- diversity(div[,3:35]) / log(specnumber(div[,3:35]))
>>>>>>> 1f94eb38000bb55c52f7e610d6d1e19e817c37e4

# calculate Shannon index using vegan package
data_shannon <- diversity(div[,3:35], index = "shannon")

# calculate Shannon index using vegan package
data_simpson <- diversity(div[,3:35], index = "simpson")
...
shan_com <- gls(data_shannon ~ Community*Site, data = data_alphadiv,na.action=na.omit)
anova(shan_com,type="marginal")
...
shan_comsite <- gls(data_shannon ~ Community_Site, data = data_alphadiv,na.action=na.omit) 
anova(shan_comsite,type="marginal")
...
chao <- gls(S.chao1 ~ Community*Site, data = data_alphadiv,na.action=na.omit)
anova(chao,type="marginal")
...
sim_com <- gls(data_simpson ~ Community*Site, data = data_alphadiv,na.action=na.omit)
anova(sim_com)
